...
export function FastHappyPaletteWithRand(colorsCount: number, rand: RandInterface): Color[] {
  ...
  for (let i = 0; i < colorsCount; i++) {
    colors[i] = Hsv(i * (360.0 / colorsCount), 0.8 + rand.float64() * 0.2, 0.65 + rand.float64() * 0.2);
  }
  return colors;
}
...
export function HappyPaletteWithRand(colorsCount: number, rand: RandInterface): [Color[], Error | null] {
  const pimpy = (l: number, a: number, b: number): boolean => {
    const [, c] = LabToHcl(l, a, b);
    return 0.3 <= c && 0.4 <= l && l <= 0.8;
  };
  return SoftPaletteExWithRand(colorsCount, { CheckColor: pimpy, Iterations: 50, ManySamples: true }, rand);
}
